<template>
  <div class="component-upload-image">
    <el-upload 
      ...
      :class="{ hide: this.fileLists.length >= this.limit }">
...
export default {
  ...
  methods: {
    ...
    handleUploadSuccess(res, file) {
      if (res.code === 0) {
        this.uploadList.push({ objectName: res.data.objectName, url: res.data.objectViewUrl });
    ...
    uploadedSuccessfully() {
      if (this.number > 0 && this.uploadList.length === this.number) {
        this.fileLists = this.fileLists.concat(this.uploadList);
        this.uploadList = [];
        this.number = 0;
        let imgs = this.listToString(this.fileLists);
        // console.log('设置图片标识')
    ...
    listToString(list, separator) {
      ...
      separator = separator || ",";
      for (let i in list) {
        strs += list[i].objectName + separator;
      }
