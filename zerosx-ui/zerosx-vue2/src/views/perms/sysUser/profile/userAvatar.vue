...
import { uploadAvatar } from "@/api/perms/sysUser";
    ...
    uploadImg() {
      this.$refs.cropper.getCropBlob(data => {
        ...
        uploadFile(formData).then(response => {
          this.uploadAvatarMethod(response.data);
        }).catch(err => {
          this.uploading = false;
        });
      });
    },
    uploadAvatarMethod(data) {
      let parma = {
        avatar: data.objectName
      };
      uploadAvatar(parma).then(res => {
        //console.log(res)
        this.open = false;
        this.options.img = data.objectViewUrl;
