...
public class OAuth2TokenServiceImpl implements IOAuth2TokenService {
    ...
    private List<TokenVO> getTokenVOS(String key, int[] startEnds) {
        ...
        for (String tokenValue : objects) {
            OAuth2Authorization oAuth2Authorization = redissonOpService.hGet(TokenStoreConstants.SAS_ACCESS_TOKEN, tokenValue, serializationCodec);
    ...
    private String rmExpireTokens(String clientId) {
        String key = TokenStoreConstants.SAS_TOKEN_PAGE + clientId;
    ...
    public boolean cleanTokenData(TokenQueryVO tokenQueryVO) {
        ...
        if ("0".equals(opType)) {
            redissonOpService.hDel(TokenStoreConstants.SAS_ACCESS_TOKEN, serializationCodec);
            redissonOpService.hDel(TokenStoreConstants.SAS_REFRESH_TOKEN, serializationCodec);
            redissonOpService.delByPattern(TokenStoreConstants.SAS_TOKEN_PAGE + "*");
        } else {
            Collection<String> resColl = redissonOpService.zGetByScore(TokenStoreConstants.SAS_TOKEN_PAGE + tokenQueryVO.getClientId(), (double) new Date().getTime() / 1000, serializationCodec);
    ...
    public void excelExport(RequestVO<TokenQueryVO> requestVO, HttpServletResponse response) {
        ...
        String key = TokenStoreConstants.SAS_TOKEN_PAGE + vo.getClientId();
        //总数量
        int totalCount = redissonOpService.zLen(TokenStoreConstants.SAS_TOKEN_PAGE + vo.getClientId(), serializationCodec);
        if (totalCount <= 0) {
            return;
        }
